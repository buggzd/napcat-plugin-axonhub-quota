...
构建生成 `dist/` 和 `napcat-plugin-axonhub-quota.zip`，含图片 worker、渲染器及第三方声明。`deploy` 从本地 Docker 容器读取现有 NapCat WebUI 凭据：旧版通过 ZIP 导入；新版在插件已安装且获准加载时，备份现有插件目录，临时停用本插件，更新文件后重新启用。不会重启容器或其他插件，也不打印凭据。新版首次安装仍须先按下文配置白名单和插件目录。
...
- 登录失效重新登录并重试一次；密码错误冷却 30 秒。GraphQL 读取因超时或网络异常失败时，仅在这次用户查询中额外重试一次；登录网络异常不会误触发密码错误冷却。失败时不回退到过期缓存。日志只记请求阶段、错误类别与耗时，不记令牌、请求体或原始响应。
